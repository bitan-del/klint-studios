...
export interface SharedActions {
    ...
    clearError: () => void;
    generateAsset: (user: User | null, onGenerationComplete: (count: number) => Promise<void>, overridePrompt?: string, images?: string[]) => Promise<void>;
    ...
    // Storage Actions
    saveGeneratedImages: (user: User | null, studioMode: StudioMode, prompt?: string) => Promise<void>;
}
...
const initialSharedState: SharedState = {
    ...
    chatHistory: (() => {
        ...
        return [
            { role: 'model', text: "Hi! I'm Chason, your Design AGI. How can I help you with your photoshoot today? You can ask me things like 'How do I use my own model?' or 'What are scene templates?'" }
        ];
    })(),
    isBotReplying: false,
    language: 'en',
...
export const createSharedSlice: StudioStoreSlice<SharedSlice> = (set, get) => ({
    ...
    generateAsset: async (user, onGenerationComplete, overridePrompt, images) => {
        const now = Date.now();
        ...
        try {
            ...
            switch (studioMode) {
                case 'chason': {
                    const { numberOfImages, aspectRatio } = get();
                    const prompt = overridePrompt || "A creative design concept";
                    ...
                    await get().saveGeneratedImages(user, 'chason', `Chason Design Generation`);
    ...
    askChatbot: async (question, images) => {
        ...
        try {
            ...
            const shouldUseStoredContext = isFollowUpRequest && hasLastContext && (!images || images.length === 0);
            ...
            set(state => {
                const newHistory = [...state.chatHistory, { role: 'model' as const, text: "Sorry, I encountered an error. Please try again." }];
                try {
                    localStorage.setItem('klint_chat_history', JSON.stringify(newHistory));
                } catch (e) {
                    console.error('Failed to save chat history to localStorage:', e);
    ...
    resetChat: () => {
        set({ chatHistory: [], chatReferenceImages: [], lastImageGenerationContext: null });
        try {
