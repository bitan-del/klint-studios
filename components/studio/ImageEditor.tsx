        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-lg z-50 flex items-center justify-center p-4 animate-fade-in duration-300">
            <div className="bg-zinc-925/70 backdrop-blur-xl rounded-xl border border-white/10 shadow-glass w-full max-w-6xl h-full max-h-[90vh] flex flex-col shadow-glass-inset">
                ...
                <div className="flex-shrink-0 flex items-center justify-between p-4 border-b border-white/10">
                    <h2 className="text-lg sm:text-xl font-bold flex items-center gap-3"><Wand2 size={20} className="text-emerald-400" />Generative Edit</h2>
                ...
                <div className="flex-grow flex flex-col lg:flex-row gap-6 p-4 sm:p-6 min-h-0">
                    ...
                    <div className="w-full lg:w-[340px] flex-shrink-0 flex flex-col space-y-4 overflow-y-auto pr-2">
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                {apparel.map(item => (
                                    <button 
                                      ...
                                      className={`aspect-square rounded-md overflow-hidden transition-all duration-200 border-2 ${selectedApparel?.id === item.id ? 'border-emerald-500 scale-105 shadow-lg' : 'border-transparent hover:border-zinc-600'}`}
                        ...
                        <div className="p-4 rounded-lg bg-zinc-900/50 border border-white/10 space-y-4 shadow-glass-inset">
                             <label className="text-base font-semibold text-zinc-100 flex items-center gap-2">
                                <Brush size={18} /> 3. Draw a mask
                            </label>
                            <p className="text-xs text-zinc-400 -mt-2 mb-3">Paint over the area you want to change.</p>
                            <div className="space-y-4">
                                <div>
                                    <div className="flex justify-between items-center">
                                        <label htmlFor="brush-size" className="text-sm font-medium text-zinc-300">Brush Size</label>
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={handleUndo} disabled={history.length <= 1} className="flex-1 flex items-center justify-center gap-2 px-3 py-1.5 rounded-md bg-zinc-800 hover:bg-zinc-700 text-sm font-semibold disabled:opacity-50 transition-colors text-zinc-300 hover:text-white border border-white/10 shadow-glass-inset"><Undo size={16} /> Undo</button>
                                    <button onClick={handleClear} className="flex-1 flex items-center justify-center gap-2 px-3 py-1.5 rounded-md bg-zinc-800 hover:bg-zinc-700 text-sm font-semibold disabled:opacity-50 transition-colors text-zinc-300 hover:text-white border border-white/10 shadow-glass-inset"><Trash2 size={16} /> Clear</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Image */}
                    <div className="flex-grow flex items-center justify-center min-h-0 bg-black/30 rounded-lg overflow-hidden relative border border-white/10">
                        <div className="relative w-full h-full flex items-center justify-center">
                            <img ref={imageRef} src={imageToEdit} alt="Editing image" onLoad={handleImageLoad} className="max-w-full max-h-full object-contain block" style={{ opacity: 0.8 }} />
                            <canvas
                                ref={canvasRef}
                ...
                {/* Footer */}
                <div className="flex-shrink-0 flex flex-col sm:flex-row items-center justify-between p-4 border-t border-white/10 gap-4">
                    {error && <p className="text-sm text-red-400 flex-1 text-left">{error}</p>}
                    <div className="flex-1 hidden sm:block"></div>
                    <div className="w-full sm:w-auto flex items-center justify-end gap-3">
                        <button 
                            onClick={cancelEditing} 
                            className="px-6 py-2.5 rounded-md bg-zinc-800 hover:bg-zinc-700 text-sm font-semibold transition-colors text-zinc-300 hover:text-white border border-white/10"
                        >
                            Cancel
                        </button>
                        <button 
                            ...
                            className="px-6 py-2.5 rounded-md bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold transition-colors flex items-center gap-2 disabled:bg-zinc-600 disabled:cursor-not-allowed"
                        >
                            {isApplyingEdit ? <Loader2 size={18} className="animate-spin" /> : <Wand2 size={18} />}
