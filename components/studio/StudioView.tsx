...
import { ChasonWorkflow } from '../chason/ChasonWorkflow';
...
export const StudioView: React.FC = () => {
    ...
    // Design mode is handled above, so we don't need to check for it here
    const showEmptyState = !isGenerating && (!generatedImages || generatedImages.length === 0) && !previewImage;
    const showPreview = !isGenerating && (!generatedImages || generatedImages.length === 0) && previewImage;
        ...
        <div className="relative w-full h-full flex flex-col gap-3">
            <div className="relative flex-1 bg-zinc-925/50 backdrop-blur-sm rounded-lg flex items-center justify-center p-2 sm:p-4 border border-white/10 shadow-2xl shadow-black/50 overflow-hidden">
                {(() => {
                    ...
                    // Design preview handled in early return
                    ...
                    <div className="h-32 bg-zinc-900/60 backdrop-blur-xl rounded-lg border border-white/10 p-3 flex items-center gap-3 overflow-x-auto model-filter-scrollbar">
                        {imagesToDisplay && imagesToDisplay.map((imageB64, index) => (
                            imageB64 ? (
                                <div
                                    key={index}
                                    className={`relative group h-full aspect-[3/4] rounded-md cursor-pointer transition-all duration-200 overflow-hidden flex-shrink-0 ${index === activeImageIndex ? 'ring-2 ring-offset-2 ring-emerald-500 ring-offset-zinc-925 shadow-lg shadow-emerald-700/30' : 'hover:opacity-80'}`}
