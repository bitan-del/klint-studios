    ...
    return (
        <label 
            ...
            className={cn(
                ...
                isDragOver ? "border-violet-500 bg-violet-500/10" : "border-zinc-700 bg-zinc-900/50 hover:border-zinc-600"
            )} 
            onDrop={handleDrop} 
        ...
        >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-zinc-500 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
            ...
            <div className="aspect-[4/5] w-full bg-zinc-925 rounded-lg border border-white/10 flex items-center justify-center text-zinc-500 text-center relative overflow-hidden group shadow-lg">
                ...
                {status === 'error' && (
                    <div className="p-4 text-red-400">
                        ...
                        <button onClick={onRetry} className="text-sm bg-red-600/20 text-red-300 px-3 py-1 rounded-md hover:bg-red-600/40">Retry</button>
                    </div>
                )}
                ...
                {status === 'done' && imageUrl && (
                    <div className="absolute top-2 right-2 z-10 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={onDownload} className="p-2 bg-zinc-800/80 rounded-full text-zinc-200 hover:bg-zinc-700" aria-label="Download">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                            </svg>
                        </button>
                        <button onClick={onRetry} className="p-2 bg-zinc-800/80 rounded-full text-zinc-200 hover:bg-zinc-700" aria-label="Regenerate">
...
export default function OneToManyProductSceneGenerator({ onBack }: { onBack: () => void }) {
    ...
    const renderConfigView = () => (
        <div className="w-full grid md:grid-cols-2 gap-8 items-start">
            ...
            <div className="bg-zinc-900/70 border border-white/10 rounded-xl p-5 shadow-lg flex flex-col gap-3">
                <div className={cn(!uploadedImage && "opacity-50 pointer-events-none")}>
                    ...
                    <p className="text-zinc-400 text-sm mb-3">Choose one or more angles to generate.</p>
                ...
                <button 
                    ...
                    className="w-full mt-4 flex items-center justify-center gap-2 text-white font-semibold text-sm py-3 px-6 rounded-lg bg-gradient-to-r from-emerald-500 to-cyan-500 hover:from-emerald-600 hover:to-cyan-600 disabled:bg-zinc-700 disabled:text-zinc-500 disabled:cursor-not-allowed transition-all duration-300 transform hover:scale-105"
                >
                    {generateButtonText}
                </button>
            </div>
    ...
    const renderResultView = () => (
        <div className="w-full flex flex-col items-center gap-8">
            ...
            <button 
                ...
                className="font-semibold text-sm text-center text-zinc-200 bg-zinc-800 hover:bg-zinc-700 border border-white/10 py-2.5 px-6 rounded-lg transition-all duration-300 hover:scale-105 mt-8"
            >
                Start Over
            </button>
        </div>
    ...
    return (
        <main className="bg-zinc-950 text-zinc-200 h-full w-full flex flex-col items-center p-4 relative font-sans overflow-y-auto">
            <div className="w-full max-w-7xl mx-auto flex flex-col items-center z-10">
                <header className="w-full flex justify-between items-center py-4 mb-6">
                    <button 
                        onClick={view === 'config' ? onBack : handleStartOver} 
                        className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors bg-zinc-900 text-zinc-300 hover:bg-zinc-800 border border-zinc-800"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" />
                        </svg>
                        {view === 'config' ? 'Back to Tools' : 'Start Over'}
                    </button>
