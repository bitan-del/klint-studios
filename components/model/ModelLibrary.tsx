            ...
            <div className="flex-shrink-0 mb-3 space-y-3">
                 <div className="flex items-center gap-2 overflow-x-auto pb-2 -mx-4 px-4 model-filter-scrollbar">
                    {MODEL_REGIONS.map(region => (
                        <button
                            ...
                            className={`px-3 py-1 text-xs font-medium rounded-full transition-all duration-200 whitespace-nowrap border ${
                                activeFilter === region
                ...
                >
                    <label 
                        ...
                        className={`text-xs font-medium text-zinc-300 flex items-center ${!canUseBatchMode ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                    >
                        Batch Mode
                        {!canUseBatchMode && <Lock size={12} className="inline-block ml-1.5 text-emerald-400" />}
                    </label>
                   <ToggleSwitch
