...
export const ModelSelectionPanel: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex-shrink-0 bg-zinc-900 p-1 rounded-full flex items-center gap-1 mb-3 border border-zinc-800 shadow-inner-soft">
                <TabButton tabId="upload" activeTab={activeTab} onClick={handleTabSwitch} icon={<UploadCloud size={14} />} label="Upload" disabled={isBatchMode}/>
                <TabButton tabId="prompt" activeTab={activeTab} onClick={handleTabSwitch} icon={<TextCursorInput size={14} />} label="Describe" disabled={isBatchMode} />
                <TabButton tabId="models" activeTab={activeTab} onClick={handleTabSwitch} icon={<Users size={14} />} label="Library" />
            </div>
            <div className="flex-grow min-h-0 flex flex-col">
                ...
                {activeTab === 'models' && (
                    <div className="flex flex-col h-full animate-fade-in">
                        ...
                        <div className="flex-shrink-0 bg-zinc-850 p-1 rounded-full flex items-center gap-1 mb-3 border border-zinc-700 shadow-inner-soft">
                             <TabButton tabId="library" activeTab={activeModelSource} onClick={handleModelSourceSwitch} icon={<Users size={14} />} label="Library" />
                             <TabButton tabId="my_agency" activeTab={activeModelSource} onClick={handleModelSourceSwitch} icon={<Star size={14} />} label="My Agency" />
                        </div>
