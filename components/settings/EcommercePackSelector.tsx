...
export const EcommercePackSelector: React.FC = () => {
    const { ecommercePack, setEcommercePack, isSocialMediaPack, t } = useStudio();
        ...
        >
            <fieldset disabled={!canUsePackshots || isSocialMediaPack}>
                <div className={`flex items-center justify-between ${!canUsePackshots || isSocialMediaPack ? 'opacity-50' : ''}`}>
                    ...
                    >
                        <Package size={18} /> {t('ecommerce_pack')}
                        {!canUsePackshots && <Lock size={12} className="inline-block ml-1.5 text-emerald-400" />}
                    </label>
                </div>
                <p className={`text-xs text-zinc-400 mb-3 ${!canUsePackshots || isSocialMediaPack ? 'opacity-50' : ''}`}>
                    {t('ecommerce_pack_desc')}
                </p>

                <div className="flex-shrink-0 bg-zinc-900 p-1.5 rounded-full flex items-center gap-1 border border-zinc-800 shadow-inner-soft overflow-x-auto model-filter-scrollbar">
                        ...
                        const name = packKey === 'none' ? 'Off' : pack.name.replace(' Pack', '');
                         return (
                            <button
                                ...
                                className={`flex-grow flex-shrink-0 whitespace-nowrap px-3 py-1.5 text-xs font-medium rounded-full transition-colors h-8 ${
