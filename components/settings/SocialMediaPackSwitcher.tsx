...
export const SocialMediaPackSwitcher: React.FC = () => {
    const { isSocialMediaPack, setIsSocialMediaPack, ecommercePack, t } = useStudio();
        ...
        >
            <fieldset disabled={!canUseSocialPack || ecommercePack !== 'none'}>
                <div className={`flex items-center justify-between ${!canUseSocialPack || ecommercePack !== 'none' ? 'opacity-50' : ''}`}>
                    ...
                    >
                        <Share2 size={18} /> {t('social_media_pack')}
                        {!canUseSocialPack && <Lock size={12} className="inline-block ml-1.5 text-emerald-400" />}
                    </label>
                    <ToggleSwitch
