...
export const ProductSceneSelector: React.FC = () => {
    ...
    return (
        <div className="flex flex-col space-y-6">
            ...
            />
            {productControls.surface.id === 'custom' && (
                <div className="animate-fade-in -mt-4 relative">
                    <textarea
                        value={productControls.customSurface}
                        onChange={(e) => updateProductControl('customSurface', e.target.value)}
                        placeholder={'e.g., placed on a bed of black volcanic sand.'}
                        rows={3}
                    ...
                    />
                    <PromptOptimizer
                        prompt={productControls.customSurface}
                        setPrompt={(v) => updateProductControl('customSurface', v)}
                        context="A custom surface for a product photoshoot"
            ...
            <div>
                <h3 className="text-sm font-semibold text-zinc-300 mb-3">{t('themed_scenes')}</h3>
                ...
                <div>
                     ...
                     <div
                        ...
                        className={`relative h-20 rounded-lg cursor-pointer transition-all duration-200 flex items-center justify-center text-center text-xs p-1 group
                            ${scene.background.id === 'suggested' ? 'ring-2 ring-offset-2 ring-emerald-500 ring-offset-zinc-925' : 'hover:scale-105'}`}
