...
const CompanionAssetsManager: React.FC = () => {
    ...
    return (
        <div>
            <p className="text-sm font-medium text-zinc-300 mb-2">Other Assets</p>
             <p className="text-xs text-zinc-500 mb-3">Add packaging or other objects to stage them together. Drag them on the canvas above.</p>
                ...
                {companionAssets.map((asset) => (
                    <div key={asset.id} className="relative group w-20 h-20 flex-shrink-0">
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity rounded-md flex items-center justify-center">
                            <button onClick={() => removeCompanionAsset(asset.id)} className="text-white hover:text-red-400 transition-colors p-1" aria-label={`Remove asset`}>
                                <Trash2 size={20} />
                            </button>
                        </div>
                    </div>
                ))}
                <div {...getRootProps()} className={`w-20 h-20 flex-shrink-0 border-2 border-dashed rounded-md flex flex-col items-center justify-center text-zinc-500 cursor-pointer transition-colors ${isDragActive ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' : 'border-zinc-700 hover:border-zinc-600'}`}>
                    <input {...getInputProps()} />
                    <PlusCircle size={20} />
        ...
        <div className="h-full flex flex-col space-y-6 overflow-y-auto pr-1">
            ...
            <div className="border-t border-zinc-800 pt-6">
                ...
                <textarea
                    ...
                    rows={3}
                    className="w-full p-3 rounded-lg bg-zinc-925 text-zinc-300 border border-zinc-700 focus:ring-1 focus:ring-emerald-500/50 focus:border-emerald-500 transition-colors duration-200 text-sm shadow-inner-soft"
            ...
            <div className="p-3 rounded-lg bg-zinc-800/50 border border-white/10 space-y-3">
                 ...
                 <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-md text-sm font-semibold transition-colors duration-200 border bg-emerald-600/80 hover:bg-emerald-600 text-white border-transparent disabled:opacity-60 disabled:cursor-wait"
                >
