...
export const ApparelUploader: React.FC = () => {
    ...
    return (
        <div className="h-full flex flex-col">
            <div {...getRootProps()} id="apparel-uploader-dropzone" className={`flex-shrink-0 flex flex-col items-center justify-center border-2 border-dashed rounded-lg p-6 cursor-pointer transition-all duration-200 mb-4 ${isDragActive ? 'border-emerald-500 bg-emerald-500/10 shadow-glow-md' : 'border-zinc-700 hover:border-zinc-600'}`}>
                <input {...getInputProps()} />
                <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-2 transition-colors ${isDragActive ? 'bg-emerald-500/20' : 'bg-zinc-800'}`}>
                    <Layers className={`transition-colors ${isDragActive ? 'text-emerald-300' : 'text-zinc-400'}`} size={24} />
                </div>
                <p className="text-zinc-300 text-center font-semibold">
                    {isDragActive ? t('drop_images_here') : t('add_clothing_images')}
                </p>
                <p className="text-sm text-zinc-500 mt-1">{t('drop_one_or_more')}</p>
            </div>
            ...
            <div className="flex items-center justify-between mb-3 px-2">
                ...
                {apparel.length > 1 && (
                    <button 
                        ...
                        className="flex items-center gap-1.5 text-xs font-semibold text-emerald-300 hover:text-emerald-200 disabled:opacity-50 disabled:cursor-wait transition-colors"
                    >
