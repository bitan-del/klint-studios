...
const ASPECT_RATIOS = [
    ...
    // { value: '4:3', label: 'Landscape (4:3)', icon: '▭' }, // Removed as requested
...
export const SimpleModeSidebar: React.FC<SimpleModeSidebarProps> = ({
    ...
    return (
        <>
            {/* Mobile Overlay */}
            <div
                className={`fixed inset-0 bg-black/80 z-40 lg:hidden transition-opacity duration-300 backdrop-blur-sm ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
                    }`}
                onClick={onClose}
            />

            <div className={`
                fixed inset-y-0 left-0 z-50 w-80 bg-zinc-900 border-r border-zinc-800 h-full flex flex-col overflow-y-auto custom-scrollbar transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 shadow-2xl lg:shadow-none
